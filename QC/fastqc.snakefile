...
rule all:
	input:
		...
		expand(fastqc_directory + "{sample}_trimmomatic_trimmed_paired_1_fastqc.html", sample=config["TCGA_LIHC_RNAseq_samples"]),
		expand(fastqc_directory + "{sample}_trimmomatic_trimmed_paired_1_fastqc.html", sample=config["TCGA_LIHC_RNAseq_samples"]),
		(fastqc_directory + "bbduk_multiqc_report.html"),
		(fastqc_directory + "trimmomatic_multiqc_report.html")
...
rule fastqc_analysis_trimmomatic_trimmed_paired:
	...
	output:
		...
		html2 = trimmed_fastqc_directory + "{sample}_trimmomatic_trimmed_paired_1_fastqc.html"
	params:
		fastqc = fastqc_path
	shell:
		"""
...
rule multiqc_trimmomatic_trimmed_paired:
	input:
	output:
...
rule multiqc_bbduk_trimmed_paired:
	input:
	output:
	...
	shell:
		"""
		multiqc {params.fastqc_dir}*bbduk*_fastqc.zip --outdir {params.output_dir} --interactive --verbose;
		mv {params.output_dir}multiqc_report.html {params.output_dir}trimmomatic_multiqc_report.html
		"""
