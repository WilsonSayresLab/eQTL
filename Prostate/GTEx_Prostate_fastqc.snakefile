...
configfile: "GTEx_Prostate_v1.5.config.json"
...
# Directory variables
fastq_directory = "/mnt/storage/public/dbgap-8834/prostate/"
adapter_directory = "/mnt/storage/CANCER_DOWNLOADS/PROCESSED/GTEx_Prostate/RNAseq/adapters/"
fastqc_directory = "/mnt/storage/CANCER_DOWNLOADS/PROCESSED/GTEx_Prostate/RNAseq/fastqc/"
trimmed_fastqs = "/mnt/storage/CANCER_DOWNLOADS/PROCESSED/GTEx_Prostate/RNAseq/trimmed_fastqs/"
trimmed_fastqc_directory = "/mnt/storage/CANCER_DOWNLOADS/PROCESSED/GTEx_Prostate/RNAseq/fastqc/trimmed/"

rule all:
	input:
		expand(fastqc_directory + "{sample}_fq1_fastqc.html", fastqc_directory = fastqc_directory, sample=config["Prostate_RNA"]),
		expand(fastqc_directory + "{sample}_fq2_fastqc.html", fastqc_directory = fastqc_directory, sample=config["Prostate_RNA"]),
		(fastqc_directory + "multiqc_report.html"),
		expand(trimmed_fastqs + "{sample}_trimmomatic_trimmed_paired_1.fastq", trimmed_fastqs=trimmed_fastqs, sample=config["Prostate_RNA"]),
		expand(trimmed_fastqs + "{sample}_trimmomatic_trimmed_unpaired_1.fastq", trimmed_fastqs=trimmed_fastqs, sample=config["Prostate_RNA"]),
		expand(trimmed_fastqs + "{sample}_trimmomatic_trimmed_paired_2.fastq", trimmed_fastqs=trimmed_fastqs, sample=config["Prostate_RNA"]),
		expand(trimmed_fastqs + "{sample}_trimmomatic_trimmed_unpaired_2.fastq", trimmed_fastqs=trimmed_fastqs, sample=config["Prostate_RNA"]),
		expand(trimmed_fastqs + "{sample}_trimmomatic.log", trimmed_fastqs=trimmed_fastqs, sample=config["Prostate_RNA"]),
		expand(trimmed_fastqc_directory + "{sample}_trimmomatic_trimmed_paired_1_fastqc.html", sample=config["Prostate_RNA"]),
		expand(trimmed_fastqc_directory + "{sample}_trimmomatic_trimmed_paired_2_fastqc.html", sample=config["Prostate_RNA"]),
...
rule trimmomatic:
	input:
		fq1 = lambda wildcards: fastq_directory + wildcards.sample + "/" + wildcards.sample + "_fixed_1.fastq",
		fq2 = lambda wildcards: fastq_directory + wildcards.sample + "/" + wildcards.sample + "_fixed_2.fastq",
		ADAPTER_FASTA = "/home/dlevy2/eQTL/Prostate/QC/adapter_sequences.fa"
	output:
